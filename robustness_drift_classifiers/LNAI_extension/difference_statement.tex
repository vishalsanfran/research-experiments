...
stability (ESI\,$\approx$\,0.06--0.16 across datasets), showing that prediction
robustness and explanation stability are \emph{decoupled}---a high RI does not
guarantee trustworthy explanations under attack.  To confirm the RF\,>\,XGB ESI
ordering is not itself a ZOO artefact, we recompute ESI under Square Attack on
Phishing: the ordering holds (RF\,ESI\,=\,0.230, XGB\,ESI\,=\,0.117 vs.\
0.287 and 0.111 under ZOO), confirming the gap is architectural.
...
comparison in a single view.  Eight new figures accompany the extended experiments:
...
heatmaps for both tree models on Phishing, NF-ToN-IoT robustness curves, the PGD
step-size ablation, and the ESI drift curves comparing ZOO versus Square Attack.
